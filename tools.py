...
#       print " Hello"
    ...
    #pdb.set_trace()
    if type(Z[0])==np.string_ or type(Z[0])==str:
        # Map to integer labels...
        # print " Hello"
...
#lets throughly test our classifier on a larget set of sample points by generating random points within the training data range...

def get_spherical_data(N = 2000,	D = 2, 	K = 3, visualize=True):
	"""
...
#lets throughly test our classifier on a larget set of sample points by generating random points within the training data range...
def plot_points_range(  model, X,Y, npts=10000, colors=['r','g','b','y','l','k']):
    """
    ...
    """
    ax=plt.gca()
    x0spr = max(X[:,0])-min(X[:,0])# range of first dimension
    x1spr = max(X[:,1])-min(X[:,1])# range of second dimension
    ...
    cs= model.predict(tx); # send the transpose of X during evaluation as well...
    
    uclasses=list(np.unique(cs))
    print (uclasses)
    ax.scatter(tx[:,0],tx[:,1],c=list(map(lambda x:colors[uclasses.index(x)],cs)), alpha=.25)
    ax.legend([str(c) for c in uclasses])
    
    ax.hold(True)
    ax.scatter(X[:,0],X[:,1],
                  c=list(map(lambda x:colors[uclasses.index(x)],Y)), 
